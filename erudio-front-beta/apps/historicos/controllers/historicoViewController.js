    ...
    class HistoricoViewController {
        ...
        buscarTodasDisciplinas(etapaCursada, disciplinaCursada, $index) {
            ...
            this.disciplinaService.getAll({etapa: etapa.id, incluirNaoOfertadas: true }).then((disciplinas) => {
                ...
                this.modalDiscplina = this.mdDialog.show({locals: {attrs: options }, controller: this.modalDisciplinasControl, templateUrl: this.erudioConfig.dominio+'/apps/historicos/partials/adicionar-disciplina.html', parent: angular.element(document.body), targetEvent: event, clickOutsideToClose: true});
                this.timeout(() => {
        ...
        abrirMediaParcial(cursada) {
            this.mediaService.getAll({disciplinaCursada: cursada.id},true).then((medias) => {
                ...
                this.mdDialog.show({locals: {attrs: {disciplina: cursada, medias: mediasParciais, titulos: titulos, config: this.erudioConfig} }, controller: this.modalMediasControl, templateUrl: this.erudioConfig.dominio+'/apps/historicos/partials/medias-parciais.html', parent: angular.element(document.body), targetEvent: event, clickOutsideToClose: true});
            });
        }
