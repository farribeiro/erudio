(function (){
    ...
    class PessoaFormController {
        constructor(service, util, erudioConfig, routeParams, $timeout, estadoService, cidadeService, enderecoService, telefoneService, $scope, beneficioService, necessidadeEspecialService, usuarioService, $http){
            this.service = service; this.http = $http; this.scope = $scope; this.util = util; this.novo = true; this.cidade = null;
            ...
            this.novaSenha = null; this.timeout = $timeout; this.scope.perfil = null;
            this.nomeForm = "pessoaForm"; this.tiposTelefone = ['CELULAR','COMERCIAL','RESIDENCIAL']; this.mapa = {}; this.abaAtiva = 0;
            this.temFoto = false; this.fotoSrc = null; this.permissaoLabel = "PESSOA"; this.titulo = "Pessoas"; this.linkModulo = "/#!/pessoas/"; this.iniciar();
        }
        ...
        preparaForm() {
            ...
            this.form = this.util.getInputBlockCustom('pessoas','form');
            this.formCards =[
                {label: 'Foto', href: this.util.getInputBlockCustom('pessoas','foto')},
                {label: 'Informações Pessoais', href: this.util.getInputBlockCustom('pessoas','informacoesPessoais')},
                {label: 'Contatos', href: this.util.getInputBlockCustom('pessoas','contatos')},
                {label: 'Endereço', href: this.util.getInputBlockCustom('pessoas','endereco')}
            ];
            this.formCardsDocumentos =[
                {label: 'Documentos', href: this.util.getInputBlockCustom('pessoas','documentos')},
                {label: 'Particularidades', href: this.util.getInputBlockCustom('pessoas','particularidades')}
            ];
            this.formCardsUsuario =[
                {label: 'Usuário', href: this.util.getInputBlockCustom('pessoas','usuario')},
            ];
            this.forms = [
        ...
        salvarFoto(){
            ...
            if (!this.util.isVazio(src)) {
                ...
                this.timeout(()=>{
                    ...
                    }).then(() => { this.util.toast("Foto atualizada com sucesso."); });
                },500);
            }
            this.cancelarFoto();
        }
        ...
        salvarFotoUpload() {
            this.timeout(() => {
                ...
                this.http.post(fileUrl,fd,{
                    ...
                    this.util.toast("Foto atualizada com sucesso.");
                });
            },500);
        }
        ...
        buscarPessoa() {
            ...
            var self = this; this.tipoCertidao = null; this.termo = null; this.livro = null; this.folha = null;
            if (!this.util.isNovo(this.routeParams.id)) {
                this.novo = false;
                this.service.get(this.routeParams.id).then((pessoa) => {
                    ...
                    $('#foto-perfil').change(() => { this.salvarFotoUpload() });
                    this.timeout(() => { 
                        this.util.aplicarMascaras(); this.getPasso(0); 
                        this.timeout(() => { this.initMapa(); },500);
                    },500);
                });
            } else {
                this.novo = true;
        ...
        carregarParticularidades() {
            ...
            if (!this.util.isVazio(this.scope.pessoa.particularidades)) {
        ...
        carregarNecessidades() {
            ...
            if (!this.util.isVazio(this.scope.pessoa.necessidadesEspeciais)) {
        ...
        carregarBeneficios() {
            ...
            if (!this.util.isVazio(this.scope.pessoa.beneficiosSociais)) {
        ...
        validar() {
            ...
            if (!this.util.isVazio(this.scope.pessoa.certidaoNascimento) && this.scope.pessoa.certidaoNascimento.length < 32) { this.util.toast("Certidão de Nascimento deve ter 32 dígitos."); return false; }
            if (obrigatorios) { return true; } else { return false; }
        }
        
        salvar() {
            if (this.responsavel === "Filiação 1") {
                this.scope.pessoa.responsavelNome === this.scope.pessoa.nomeMae;
            } else if (this.responsavel === "Filiação 2") {
                this.scope.pessoa.responsavelNome === this.scope.pessoa.nomePai;
            } else if (this.responsavel === "Não se aplica") {
                this.scope.pessoa.responsavelNome === "nenhum";
            }
            ...
            if (this.validar()) {
                ...
                if (this.novo) {
                    resultado = this.service.salvar(this.scope.pessoa,true);
                    this.novo = false;
                } else {
    ...
    PessoaFormController.$inject = ["PessoaService","Util","ErudioConfig","$routeParams","$timeout","EstadoService","CidadeService","EnderecoService","TelefoneService","$scope","BeneficioService","NecessidadeEspecialService","UsuarioService","$http"];
    angular.module('PessoaFormController',['ngMaterial', 'util', 'erudioConfig']).controller('PessoaFormController',PessoaFormController);
})();