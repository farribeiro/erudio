(function (){
    ...
    cargosDirectives.directive('inputCargo', function (){ return {
            ...
            compile: function ($element, $attributes){
                $attributes.model = 'cargo.'+$attributes.model; $attributes.campoId = $attributes.model;
                var formId = 'cargoForm';
                if ($attributes.required === "true") { $($element[0].children[0]).find('input').prop("required",true); }
                switch ($attributes.model) {
                    case 'cargo.nome':
                        $attributes.id = 'cargo_nome';
    ...
    cargosDirectives.directive('selectCargo', function (){ return {
            ...
            compile: function ($element, $attributes){
                ...
                switch ($attributes.model) {
                    case 'cargo.grupo.id':
                        $attributes.id = 'modulo_grupo';
                        $attributes.simples = true;
                        $attributes.label = 'Grupo de Permissão';
                        $attributes.icone = 'lock_outline';
                        $attributes.formError = formId + '.' + $attributes.model + '.$error';
                        $attributes.errors = [{name:'required', message: 'Este campo é obrigatório.'}];
                        $($element[0].children[0]).find('.errors-wrapper').append('<div ng-message="required">Este campo é obrigatório.</div>');
                    break;
