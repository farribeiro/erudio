...
(function () {
    ...
    var diarioNotasModule = angular.module('diarioNotasModule', ['servidorModule', 'erudioConfig']);
    diarioNotasModule.controller('DiarioNotasController', ['$scope', 'Servidor', 'Restangular', '$timeout', '$templateCache', 'makePdf', 'dateTime', 'ErudioConfig', function ($scope, Servidor, Restangular, $timeout, $templateCache, makePdf, dateTime, ErudioConfig) {
        ...
        //CARREGA TELA ATUAL
        $scope.tela = ErudioConfig.getTemplateLista('diarioNotas'); $scope.lista = true;
        //ATRIBUTOS
        $scope.titulo = "Diário de Notas"; $scope.progresso = false; $scope.cortina = false;$scope.nomeUnidade = null; $scope.turmas = []; $scope.etapas = []; $scope.unidades = []; $scope.media = {id:null}; $scope.height = $(window).height()-150;
        ...
        $scope.buscarUnidades = function (unidade) {
            $scope.mostraProgresso(); $scope.nomeUnidade = unidade; 
        ...
        $scope.selecionaEtapa = function (id) {
            if (id !== null && id !== undefined) {
                var promiseRegime = Servidor.buscarUm('etapas',id);
                promiseRegime.then(function(response){
                    $scope.medias = new Array(parseInt(response.data.sistemaAvaliacao.quantidadeMedias)); $scope.unidadeRegime = response.data.sistemaAvaliacao.regime.unidade;
                    $timeout(function(){ $('#media').material_select('destroy'); $('#media').material_select(); },500);
                });
        ...
        $scope.inicializar = function () {
            ...
            $timeout(function(){
                $('#modal-ajuda-notas').leanModal(); $('select').material_select('destroy'); $('select').material_select(); Servidor.verificaLabels();
