...
(function () {
    var certificadosModule = angular.module('certificadosModule', ['servidorModule', 'certificadosDirectives','erudioConfig']);
    //DEFINIÇÃO DO CONTROLADOR
    certificadosModule.controller('CertificadosController', ['$scope', 'Servidor', '$timeout', '$templateCache', 'ErudioConfig', '$rootScope', function ($scope, Servidor, $timeout, $templateCache, ErudioConfig, $rootScope) {
        ...
        //CARREGA TELA ATUAL
        $scope.tela = ErudioConfig.getTemplateLista('certificados'); $scope.lista = true;
        //ATRIBUTOS
        $scope.titulo = "Certificados"; $scope.progresso = false; $scope.cortina = false; $scope.unidades = []; $scope.unidadesNota = []; $scope.cursos = []; $scope.etapas = []; $scope.nomeUnidadeN = null;
        $scope.turmas = []; $scope.turmaBusca = {curso: {id:null}, etapa:{id:null}, turma:{id:null}, media:{id:null}, disciplina:{id:null}, aluno:{id:null}, diretor:{id:null} }; $scope.nomeUnidade = null; $scope.unidade = {id:null};
        ...
        $scope.getCargoDiretor = function () {
            $scope.diretores = [];
            var promise = Servidor.buscar('cargos',{nome:'Diretor'});
            promise.then(function(response){
                $scope.cargoDiretor = response.data;
                $timeout(function(){ 
                    $scope.cargoDiretor.forEach(function(diretor){
                        $scope.getDiretor(diretor);
                    });
                },500);
            });
        };
        ...
        $scope.diretores = [];
        $scope.getDiretor = function (diretor){
            var promise = Servidor.buscar('alocacoes',{'vinculo_cargo': diretor.id, instituicao: $scope.unidade.id});
            promise.then(function(response){
                if (response.data.length > 0) {
                    response.data.forEach(function(vinculo){ $scope.diretores.push(vinculo); });
                }
                $timeout(function () { $('#diretor').material_select('destroy'); $('#diretor').material_select(); $scope.fechaProgresso(); }, 500);
            });
        };
        ...
        $scope.reiniciarBusca = function () {
            ...
            $scope.nomeUnidade = null; $scope.turmaBusca = {curso: {id:null}, etapa:{id:null}, turma:{id:null}, media:{id:null}, disciplina:{id:null}, aluno:{id:null}, diretor:{id:null}}; $scope.etapas = []; $scope.disciplinasTurma = [];
                            ...
                            for (var i=0; i<$scope.atribuicoes.length; i++) {
                                ...
                                if (i === $scope.atribuicoes.length-1) {
                                    if ($scope.isAdmin) {
                                        ...
                                        if ($scope.unidades.length === 1) { $scope.unidade = $scope.unidades[0]; $scope.getCargoDiretor(); $scope.buscarCursos(); }
        ...
        $scope.buscarTurmas = function (origem) {
            $scope.mostraProgresso(); $scope.turmaBusca.turma.id = null;
                if (!$scope.unidade.id && origem === 'formBusca') {
                    ...
                    var promise = Servidor.buscar('turmas', {'unidadeEnsino': $scope.unidade.id, 'etapa': $scope.turmaBusca.etapa.id, 'curso': $scope.turmaBusca.curso.id, 'encerrado': true});
                    promise.then(function (response) {
        ...
        $scope.selecionaUnidade = function(unidade) {
            ...
            $timeout(function(){ Servidor.verificaLabels(); $scope.buscarCursos(); $scope.getCargoDiretor(); },100);
        };
        ...
        $scope.gerarCertificado = function (url,tipo) {
            if (url === undefined) {
                url = ErudioConfig.urlRelatorios+'/certificados-conclusao?turma='+$scope.turmaBusca.turma.id+"&vinculo="+$scope.turmaBusca.diretor.id;
                $scope.getCertificado(url);
            } else {
        ...
        $scope.getCertificado = function (url) {
