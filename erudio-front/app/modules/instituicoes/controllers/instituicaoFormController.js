...
(function (){
    var instituicaoFormModule = angular.module('instituicaoFormModule', ['servidorModule','instituicaoDirectives','erudioConfig']);
    instituicaoFormModule.controller('InstituicaoFormController', ['$scope', 'Servidor', 'Restangular', '$timeout', '$templateCache', 'ErudioConfig', '$routeParams', function($scope, Servidor, Restangular, $timeout, $templateCache, ErudioConfig, $routeParams) {
        ...
        //CARREGA TELA ATUAL
        $scope.tela = ErudioConfig.getTemplateForm('instituicoes'); $scope.lista = false;
        //ATRIBUTOS
        $scope.titulo = ""; $scope.progresso = false; $scope.cortina = false; $scope.instituicoes = []; $scope.instituicaoRemover = null; $scope.nomeInstituicao = '';
        $scope.pagina = 0; $scope.instituicao = { nome:null, sigla:null, cpfCnpj:null, email:null, endereco:null, telefones:[] }; $scope.acao = "Cadastrar";
        ...
        $scope.telefone = {"descricao": '', "falarCom": null, "numero": null, "pessoa": { 'id': null } }; $scope.estadoId = null; $scope.cidadeId = null;  $scope.estados = []; $scope.cidades = [];
        //ABRE AJUDA
        ...
        //PREPARA VOLTAR
        $scope.prepararVoltar = function(objeto) { if (objeto.nome && !objeto.id) { $('#modal-certeza').modal(); } else { window.location = $scope.config.dominio+"/#/instituicoes"; } };
            
        //INICIALIZAR
        $scope.inicializar = function () {
            $('.material-tooltip').remove();
            $timeout(function () {
                $('.tooltipped').tooltip({delay: 50}); $('ul.tabs').tabs(); $('#modal-ajuda-instituicao').modal(); $('.titulo-typo').html($scope.titulo);
        ...
        $scope.carregar = function () {
            $scope.mostraProgresso(); $("#telefone").mask("(00) 0000-00000");
            if ($routeParams.id === 'novo') {
                $scope.titulo = "Cadastrar Instituição"; $scope.instituicao.endereco = $scope.endereco; Servidor.verificaLabels(); $('.cnpj').mask('00.000.000/0000-09');
                $scope.instituicao.endereco.latitude = -26.929647; $scope.instituicao.endereco.longitude = -48.683661; $scope.fechaProgresso();
                $timeout(function(){ $scope.initMap(false, "mapa"); $('#nomeInstituicaoFocus').focus(); $('#descricaoInstituicao').material_select(); $('.tooltipped').tooltip({delay: 30}); }, 300);
            } else {
                $scope.titulo = "Editar Instituição"; var promise = Servidor.buscarUm('instituicoes', $routeParams.id);
                promise.then(function (response) { 
                    $scope.instituicao = response.data; $('.cnpj').mask('00.000.000/0000-09');
                    $scope.estadoId = $scope.instituicao.endereco.cidade.estado.id; $scope.cidadeId = $scope.instituicao.endereco.cidade.id; $scope.buscaCidades();
                    $scope.buscaTelefones(); $scope.fechaProgresso(); Servidor.verificaLabels();
                    $timeout(function(){ $scope.initMap(false, "mapa"); $('#nomeInstituicaoFocus').focus(); $('#descricaoInstituicao').material_select(); $('.tooltipped').tooltip({delay: 30}); }, 300);
                });
            }
        };
        ...
        //INICIALIZANDO
        $scope.inicializar(); $scope.carregar();
    }]);
})();
