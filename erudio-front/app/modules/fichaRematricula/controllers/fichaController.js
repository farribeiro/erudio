...
(function () {
    var fichaModule = angular.module('fichaModule', ['servidorModule', 'fichaDirectives','erudioConfig']);
    //DEFINIÇÃO DO CONTROLADOR
    fichaModule.controller('FichaController', ['$scope', 'Servidor', '$timeout', '$templateCache', 'ErudioConfig', '$rootScope', function ($scope, Servidor, $timeout, $templateCache, ErudioConfig, $rootScope) {
        ...
        //CARREGA TELA ATUAL
        $scope.tela = ErudioConfig.getTemplateLista('fichaRematricula'); $scope.lista = true;
        //ATRIBUTOS
        $scope.titulo = "Confirmação de Rematrícula"; $scope.progresso = false; $scope.cortina = false; $scope.unidades = []; $scope.unidadesNota = []; $scope.cursos = []; $scope.etapas = []; $scope.nomeUnidadeN = null;
        ...
        $scope.gerarBoletim = function (url,tipo) {
            if (url === undefined) {
                ...
                //else { 
                url = ErudioConfig.urlServidor+'/report/ficha-rematricula?turma='+$scope.turmaBusca.turma.id;
                $scope.getBoletim(url);
            } else {
                if (tipo === 'quali' && $scope.mediaNumero === null || $scope.mediaNumero === undefined) {
                    Servidor.customToast('Selecione uma média antes de gerar o arquivo.');
                } else {
                    $scope.getBoletim(url);
                }
            }
