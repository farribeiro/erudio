...
(function () {
    var cursoFormModule = angular.module('cursoFormModule', ['cursoDirectives', 'etapaDirectives', 'servidorModule']);
    ...
    cursoFormModule.controller('CursoFormController', ['$scope', '$timeout', 'Servidor', 'Restangular', 'CursoService', 'EtapaService', '$templateCache', '$routeParams', function ($scope, $timeout, Servidor, Restangular, CursoService, EtapaService, $templateCache, $routeParams) {
            ...
            $scope.searchCurso = function () {
                ...
                if ($routeParams.id === "novo") {
                    ...
                    var promise = Servidor.buscar('cursos',{'id': $routeParams.id});
                    promise.then(function (response) {
                        $scope.curso = response.data[0];
                        $timeout(function () {
                            $scope.carregar($scope.curso,false,false,null);
                        }, 1000);
                    });
                }
            };
