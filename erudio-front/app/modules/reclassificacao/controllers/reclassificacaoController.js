...
(function () {
    var reclassificacaoModule = angular.module('reclassificacaoModule', ['servidorModule', 'reclassificacaoDirectives']);

    reclassificacaoModule.controller('ReclassificacaoController', ['$scope', 'Servidor', 'Restangular', '$timeout', 'dateTime', '$templateCache',function ($scope, Servidor, Restangular, $timeout, dateTime, $templateCache) {
...
//            });
        ...
        $scope.selecionarCurso = function () {
            ...
            promise.then(function(response){
                if (response.data.length > 0 && response.data[0].modulo.curso.nome === "Ensino Fundamental") {
                    ...
                    $scope.etapasOfertadas = [];
                    $timeout(function(){$('#etapas-ofertadas').material_select('destroy'); $('#etapas-ofertadas').material_select();},100);
                    Servidor.customToast("Nâo é possível reclassificar neste curso.");
                }
            });
        };
        
        $scope.selecionarEtapaBusca = function () {
...
//            $scope.mostraListaMovimentacoes = false;
        ...
        $scope.buscarTurmas = function (etapaId) {
            if ($scope.opcaoEnvio === 'Movimentação') {
                ...
                var unidade = $scope.enturmacao.matricula.unidadeEnsino.id;
            } else {
...
//                    $scope.liberarVaga($scope.movimentacao.matricula.aluno.id);                    
        ...
        $scope.buscarMatriculas = function(matricula, pagina, origem) {            
            ...
            var promise = Servidor.buscar('enturmacoes', {
