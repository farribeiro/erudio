...
class EtapaCursada extends AbstractEditableEntity {
    ...
    * @JMS\Groups({"DETAILS"}) 
    * @JMS\MaxDepth(depth = 1)
    * @ORM\OneToOne(targetEntity = "Enturmacao", inversedBy = "etapaCursada")
    */
    private $enturmacao;
    ...
    */
    ...
    function __construct(Matricula $matricula, Etapa $etapa, $ano, $unidadeEnsino, $cidade,
            $status = self::STATUS_APROVADO, Enturmacao $enturmacao = null, $insercaoManual = false) {
    ...
    */   
    ...
    function getTotalFaltas() {
        if ($this->enturmacao) {
            $disciplinasCursadas = $this->enturmacao->getDisciplinasCursadas();
            return $this->etapa->getFrequenciaUnificada() 
                ? $disciplinasCursadas->first()->getTotalFaltas()
                : array_reduce($disciplinasCursadas->toArray(), function($acc, $d) { 
                    return $acc + $d->getTotalFaltas();
                }, 0);
        }
        return 0;
    }
