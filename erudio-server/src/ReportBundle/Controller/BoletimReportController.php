    ...
    */
    function turmaAction(Request $request) {
        try {
            ...
            $exame = $request->query->get('exame', 0);
            $enturmacoes = !$exame
                    ? $turma->getEnturmacoes()
                    : $turma->getEnturmacoes()->filter(function($e) {
                            return $e->getDisciplinasCursadas()->exists(function ($d) {
                                    return $d->getMedias()->count() > $d->getEtapa()->getSistemaAvaliacao()->getNumeroMedias();
                            });
                        });
            $boletins = [];
