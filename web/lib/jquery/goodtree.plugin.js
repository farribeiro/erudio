      ...
      return this.each(function() {
        ...
        $(this).find('li').each(function() {
          ...
          if($(this).children('ul').length > 0) {
            $(this).prepend($('<div />', {'id' : id, 'class': settings.classPrefix + "toggle " + settings.expandIconClass}));
          } else {
        	$(this).prepend($('<div />', {'id' : id, 'class': settings.classPrefix + "toggle empty"}));  
          }
        });
        ...
        $('.has_child').click(function() {
            ...
            $(this).parent().children('ul').toggle();
            if ($('#' + id).hasClass('open')) {
            	$('#' + id).removeClass(settings.contractIconClass).addClass(settings.expandIconClass);
            } else {
            	$('#' + id).removeClass(settings.expandIconClass).addClass(settings.contractIconClass);
            }
        });
