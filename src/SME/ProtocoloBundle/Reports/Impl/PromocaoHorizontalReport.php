...
class PromocaoHorizontalReport extends RequerimentoReport
{
    ...
    private function rel_requerimento_promocao_horizontal($via) {
        $this->titulo = 'REQUERIMENTO DE PROMOÇÃO HORIZONTAL';
        $this->AddPage();
        $this->AliasNbPages();
        ...
        $texto = 'O servidor efetivo abaixo assinado, do Quadro do Magistério Público Municipal, ' .
                'atuando na função de ' . $this->params['cargo_atual'] . ', requer a Vsª se digne conceder, PROMOÇÃO HORIZONTAL, ' .
    ...
    private function rel_promocao_funcional($via) {
        $this->titulo = 'PROMOÇÃO HORIZONTAL';
        $this->AddPage();
        $this->AliasNbPages();
        ...
        $chTotal = 0;
        for($i = 1; $i <= 8; $i++) {
            if(isset($this->params['curso_' . $i])) {
                $this->Cell(364, 14, $this->params['curso_' . $i], 1, 0, 'J', false);
                $this->Cell(118, 14, $this->params['conclusao_' . $i], 1, 0, 'C', false);
                $this->Cell(58, 14, $this->params['ch_' . $i], 1, 1, 'R', false);
                $chTotal += $this->params['ch_' . $i];
            }
        }

        //Total
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(482, 14, 'TOTAL', 1, 0, 'R', false);
        $this->Cell(58, 14, $chTotal, 1, 1, 'R', false);
        //--------------------------------------------------------------------------------------
        ...
        $this->SetFont('Arial', '', 8);
        $this->Cell(482, 10, '(Preenchido pela Chefia Imediata)', 'LBR', 0, 'C', FALSE);
        $this->Cell(58, 10, '', 'LBR', 1, 'C', FALSE);
        ...
        // Linha 1
        $this->Cell(116, 10, '( ) Favorável', 'LTR', 0, 'L', false);
        $this->Cell(424, 10, '( ) Desfavorável. MOTIVO:', 'LTR', 1, 'L', false);

        $this->Cell(116, 20, '', 'LBR', 0, 'C', false);
        $this->Cell(424, 20, '', 'LBR', 1, 'C', false);

        // Linha 2
        $this->Cell(116, 15, 'ASS. E CARIMBO', 'LTR', 0, 'C', false);
        $this->Cell(424, 15, '', 'LTR', 1, 'L', false);
    ...
    private function rel_manifestacao_comissao($via) {
        ...
        $this->SetFont('Arial', 'B', 12);
        $this->MultiCell(540, 14, 'CUMPRIMENTO DOS REQUISITOS PARA PERCEPÇÃO DA PROMOÇÃO HORIZONTAL', 0, 'C');
        $this->MultiCell(540, 14, '(ART.24 DA LEI 132/08)', 0, 'C');

        $this->Ln();

        $this->SetFont('Arial', '', 10);

        $this->MultiCell(540, 14, '[__] I - os interstícios e percentuais previstos nos Anexos I e I-A', 0, 'J');
        $this->MultiCell(540, 14, '[__] II - a obtenção da aprovação mínima necessária na avaliação de desempenho, na forma prevista no decreto que a regulamentará.', 0, 'J');
        $this->MultiCell(540, 14, '[__] III - a participação em cursos de formação continuada afins ao cargo que ocupa. § 1ª Para efeito da promoção de que trata este artigo, será considerada a participação do servidor em cursos de formação continuada com carga mínima total de 40 horas.', 0, 'J');
        ...
        $this->MultiCell(540, 14, 'POR MAIORIA com base nas portarias 001/2017, e no Decreto nº 9327/2011 que regulamenta o inciso II do art. 24 da Lei Complementar 132/2008(alterada pelas LC 194 e 195/2011),', 0, 'J');

        $this->Ln();

        $this->MultiCell(540, 14, '[__] Deferimos a Promoção Horizontal a contar de ___/___/___', 0, 'J');
        ...
        $this->SetFont('Arial', 'B', 10);
        $this->MultiCell(540, 14, '*Art. 25 O acréscimo pecuniário decorrente da promoção horizontal será pago (LC/132/2008)', 0, 'J');

        $this->SetFont('Arial', '', 10);
        $this->MultiCell(540, 14, '[__] I - Automaticamente, no mês subsequente ao término do interstício , se o servidor preencher dentro deste, também os requisitos previstos nos incisos II e III do art.24', 0, 'J');
        $this->MultiCell(540, 14, '[__] II - A contar da data de protocolização, se o servidor preencher o requisito III do art 24 após ao término do interstício.', 0, 'J');
        $this->MultiCell(540, 14, '[__] III - No mês subsequente à data em que o servidor alcançar a pontuação mínima necessária a obtenção do benefício, na hipótese de que trata o §1º do art. 24.', 0, 'J');
