...
class CancelamentoValeTransporteReport extends RequerimentoReport
{
    ...
    private function rel($via) {
        $this->titulo = 'REQUERIMENTO DE CANCELAMENTO DE VALE TRANSPORTE';
        $this->AddPage();
        $this->AliasNbPages();
        ...
        $this->Cell(10,20,' ', '', 0, 'J');
        $this->Cell(230,20,$this->params['celular'], 'LTBR', 1, 'J');
        $this->Ln(5);
        
        //----------------------------------------------------------------
        $this->SetFont('Arial','', 9);
        $this->Cell(300,10,'Empresa de transporte', '', 0, 'J');
        $this->Cell(10,10,' ', '', 0, 'J');
        $this->Cell(85,10,'Cartão SIM', '', 1, 'J');

        $this->SetFont('Arial', '', 10);
        $this->Cell(300, 20, $this->params['empresa'], 'LTBR', 0, 'J');
        $this->Cell(10,20,' ', '', 0, 'J');
        $this->Cell(230,20,$this->params['cartao_sim'], 'LTBR', 1, 'J');
        $this->Ln(5);
        //----------------------------------------------------------------
        ...
        $this->MultiCell(540, 16, 'Eu, servidor acima identificado, venho requerer a Vsª se digne conceder o CANCELAMENTO DO VALE TRANSPORTE. ' .
                'Estou ciente que para cancelar no mês seguinte, o requerimento deverá ser entregue até o dia 22 de cada mês na ' .
