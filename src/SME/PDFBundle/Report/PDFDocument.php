...
namespace SME\PDFBundle\Report;
...
abstract class PDFDocument extends \PDF_MC_TABLE {
    ...
    const FONT_DEFAULT_TYPE = 'Arial';
    const FONT_DEFAULT_SIZE = 12;
    ...
    function header($title = 'SECRETARIA DE EDUCAÇÃO', $subtitle = 'Diretoria de Gestão de Pessoas') {
        $this->setX($this->headerX);
        $this->SetY($this->headerY);
        $this->SetFont(self::FONT_DEFAULT_TYPE, 'B', 16);
        if($this->w > 250) {
            $this->Cell(140, 15, $title, 0, 1, 'L');
            $this->Image(dirname(__FILE__) . '/../Resources/images/brasao-municipal.jpg', $this->w - 70, 10, 58, 14);
            $this->SetFont(self::FONT_DEFAULT_TYPE, '', 12);
            $this->SetY(21);
            $this->Cell(140, 5, $subtitle, 0, 1, 'L');
        } else {
            $this->Cell(95, 15, $title, 'RB', 1, 'R');
            $this->Image(dirname(__FILE__) . '/../Resources/images/brasao-municipal.jpg', $this->w - 102, $this->y - 15, 58, 14);
