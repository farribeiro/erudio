...
class PromocaoVerticalReport extends PDFDocument {

    private $promocaoVertical;
    private $alocacoes;
    private $funcaoAtual;
    
    public function getPromocaoVertical() {
        return $this->promocaoVertical;
    }
    ...
    public function setPromocaoVertical($promocaoVertical) {
        $this->promocaoVertical = $promocaoVertical;
    }
    ...
    private function rel_requerimento_promocao_vertical($via) {
        ...
        $this->Ln(3);
        $this->MultiCell(190, 6, 'REQUERIMENTO DE PROMOÇÃO VERTICAL', 0, 'C');
        ...
        $this->Cell(105, 8, $this->promocaoVertical->getVinculo()->getServidor()->getNome(), 'LTBR', 0, 'J');
        $this->Cell(5, 8,'','', 0, 'J');
        $this->Cell(80, 8, DocumentosUtil::formatarCpf($this->promocaoVertical->getVinculo()->getServidor()->getCpfCnpj(), 0, ',', '.'), 'LTBR', 1, 'J');
        $this->Ln(1);
        ...
        $this->SetFont(self::FONT_DEFAULT_TYPE,'', 10);
        $this->Cell(105, 8, $this->promocaoVertical->getVinculo()->getCargo()->getNome(), 'LTBR', 0, 'J');
        $this->Cell(5, 8,'','', 0, 'J');
        $this->Cell(40, 8, $this->promocaoVertical->getVinculo()->getMatricula(), 'LTBR', 0, 'J');
        $this->Cell(5, 8,'','', 0, 'J');
        $this->Cell(35, 8, $this->promocaoVertical->getVinculo()->getDataNomeacao()->format('d/m/Y'), 'LTBR', 1, 'J');
        $this->Ln(1);
        ...
        $this->Ln(5);
        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 12);
        $funcaoAtual = count($this->alocacoes) 
        ...
        $texto = 'O servidor efetivo abaixo assinado, do Quadro do Magistério Público Municipal, ' .
                'atuando na função de ' . $funcaoAtual . ', requer a Vsª se digne conceder, PROMOÇÃO VERTICAL, ' .
    ...
    private function rel_promocao_funcional($via) {
        ...
        $this->MultiCell(0 , 8, 'REQUERENTE: ' . $this->promocaoVertical->getVinculo()->getServidor()->getNome());
        $this->Ln(1);
        ...
        $this->Cell(145, 6, 'Curso', 1, 0, 'L', true);
        $this->Cell(30, 6, 'Data de Conclusão', 1, 0, 'C', true);
        $this->Cell(15, 6, 'C.H.', 1, 1, 'C', true);

        //Linha 1
        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 8);
        $this->Cell(145, 6, $this->promocaoVertical->getNomeCurso(), 1, 0, 'J', false);
        $this->Cell(30, 6, $this->promocaoVertical->getDataConclusaoCurso()->format('d/m/Y'), 1, 0, 'C', false);
        $this->Cell(15, 6, $this->promocaoVertical->getCargaHorariaCurso(), 1, 1, 'C', false);
    ...
    private function rel_manifestacao_comissao($via) {
        ...
        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 10);
        $this->MultiCell(190, 6, 'REQUERENTE: ' . $this->promocaoVertical->getVinculo()->getServidor()->getNome(), 0, 'J');
        $this->Cell(140, 6, 'CARGO DE ORIGEM: ' . $this->promocaoVertical->getVinculo()->getCargo()->getNome(), 0, 0, 'J');
        $this->Cell(50, 6, 'MATRÍCULA: ' . $this->promocaoVertical->getVinculo()->getMatricula(), 0, 1, 'L');
        $this->MultiCell(190, 6, 'DATA DA NOMEAÇÃO: ' . $this->promocaoVertical->getVinculo()->getDataNomeacao()->format('d/m/Y'), 0, 'J');
        $this->MultiCell(190, 6, 'CURSO: ' . $this->promocaoVertical->getNomeCurso(), 0, 'J');
        $this->Ln(3);

        $this->SetFont(self::FONT_DEFAULT_TYPE, 'B', 10);
        $this->MultiCell(190, 7, 'CUMPRIMENTO DOS REQUISITOS PARA PERCEPÇÃO DA PROMOÇÃO VERTICAL (ART.27 DA LEI 132/08)', 0, 'J');

        $this->Ln(2);

        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 10);

        $this->MultiCell(190, 6, '[__] I - o interstício mínimo de 06 (seis) anos, não exigido qualquer prazo em relação à primeira progressão após a publicação desta Lei Complementar, desde que cumprido o estágio probatório.', 0, 'J');
        $this->MultiCell(190, 6, '[__] II - ter obtido a aprovação necessária na avaliação de desempenho, na forma prevista no decreto que a regulamentará.', 0, 'J');
        $this->MultiCell(190, 6, '[__] III - a apresentação das titulações exigidas no artigo 5º.', 0, 'J');
        ...
        $this->Ln(2);
        $this->MultiCell(190, 6, 'POR MAIORIA com base nas portarias 001/2017, e no Decreto nº 9327/2011, que regulamenta o inciso II do art. 27 da Lei Complementar 132/2008(alterada pelas LC 194 e 195/2011),', 0, 'J');
        $this->Ln(2);

        $this->MultiCell(190, 6, '[__] Promovemos verticalmente a contar de ___/___/___  Padrão/Nível _______________', 0, 'J');
        ...
        $this->SetFont(self::FONT_DEFAULT_TYPE, 'B', 10);
        $this->MultiCell(190, 6, '*Art. 28 O acréscimo pecuniário decorrente da promoção vertical será pago (LC/132/2008)', 0, 'J');

        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 10);
        $this->MultiCell(190, 6, '[__] I - Automaticamente, no mês subsequente ao término do interstício , se o servidor preencher dentro deste, também os requisitos previstos nos incisos II e III do art.27.', 0, 'J');
        $this->MultiCell(190, 6, '[__] II - A contar da data de protocolização, se o servidor preencher o requisito III do art 27 após ao término do interstício.', 0, 'J');
        $this->MultiCell(190, 6, '[__] III - No mês subsequente à data em que o servidor alcançar a pontuação mínima necessária a obtenção do benefício, na hipótese de que trata o §1º do art. 27.', 0, 'J');
        ...
        $this->SetFont(self::FONT_DEFAULT_TYPE, '', 10);
        $this->MultiCell(190, 6, 'Comunique-se esta decisão a(o) requerente, por resposta ao requerimento ou portaria publicada no Jornal do Município.', 0, 'J');
    ...
    function caixaProtocolo() {
        ...
        $X = 15;
        $this->SetX($X);
        $this->MultiCell($L, 6, 'Protocolo: ' . $this->promocaoVertical->getProtocolo(), 'LTR', 'J');
        $this->SetX($X);
